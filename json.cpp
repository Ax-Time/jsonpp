...
ValueNode::ValueNode(std::string value) : _value(value), _printer([](std::ostream& os, std::string value) -> std::ostream& { return os << "\"" << value << "\""; }) { }
ValueNode::ValueNode(const char* value) : ValueNode(std::string(value)) { }
ValueNode::ValueNode(bool value) : _value(value ? "1" : "0"), _printer([](std::ostream& os, std::string value) -> std::ostream& { return os << (std::stoi(value) ? "true" : "false"); }) { }
...
std::ostream& ValueNode::dump(std::ostream& os) const { 
    if (value().has_value()) return _printer(os, value().value());
    return os << "null";
}
...
std::ostream& ObjectNode::dump(std::ostream& os) const {
    ...
    for (auto it = children.begin(); it != children.end(); ++it) {
        os << "\"" << it->first << "\": ";
        it->second->dump(os);
        if (std::next(it) != children.end()) os << ", ";
    }
    return os << "}";
}

Proxy<Node> ArrayNode::operator[] (size_t idx) {
    if (idx >= children.size()) { return ValueNode::proxy(); }
    return children[idx];
}
...
Json Json::operator[](std::string key) {
    if (root->key_indexable()) return root.as<KeyIndexableNodeI>()->operator[](key);
    return Json();
}
...
Json Json::parse_object(std::string& json, size_t& index) {
    ...
    while (json[index] != '}') {
        ...
        if (json[index++] != '"') throw MalformedJson();
        while (json[index] != '"') {
            key << json[index++];
        }
...
Json Json::parse_value(std::string& json, size_t& index) {
    ...
    if (is_number(json[index])) {
        ...
        if (is_float) return Json(std::stof(number.str()));
